...
const initApp = () => {
  ...
  .then(_accounts => {
    ...
    e.preventDefault();
        ...
        crud.methods.New_entry(name,active,effect,death,recover,patient,doctor,hash).send({from: accounts[0]})
    .then(result => {
...
${result[10]}  = ${result[11]}`;
    ...
    e.preventDefault();
        const name = e.target.elements[0].value;
        crud.methods.getAll(name).send({ from: accounts[0] })
    .then(result => {
      $Detail_stateResult.innerHTML = `${name} has been updated`;
    })
    .catch(_e => {
